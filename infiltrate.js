...
// https://github.com/tomdunning/bitburner-solutions/blob/main/infiltrate.js
...
const infiltrationGames = [
  ...
  },
  {
    ...
    init: function (screen) {},
    play: function (screen) {
      function directionToKeyPress(direction) {
        switch (direction) {
          case '↑':
            pressKey('w');
            break;
      ...
      function incrementDirectionIndex(nextDirectionIndex, code) {
        if (nextDirectionIndex > code.length) {
          state.nextDirectionIndex = 0;
        } else {
          state.nextDirectionIndex = nextDirectionIndex;
        }
      }

      const h4 = getEl(screen, 'h4');
      const code = h4[1].textContent;
      const directionIndex = state.nextDirectionIndex || 0;
      directionToKeyPress(code[directionIndex]);
      incrementDirectionIndex(directionIndex + 1, code);
    },
  },
  {
